...
\begin{itemize}
\item Married two-parent families \ : \ desirable outcomes for parents and children%$\genfrac{}{}{0pt}{1}{\Leftarrow}{\Rightarrow}$
\item Pro-marriage policies: child support, tax brackets, divorce regulations, etc
\item Can economic policies create successful marriages? Love vs money.
\end{itemize}
\end{frame}

\begin{frame}

\frametitle{Kids-First Marriages are Less Stable}
\begin{enumerate}
\item{\textbf{Shotgun marriage}} --- couple marrying when woman gets pregnant
\item{\textbf{Kids-first marriage}} --- child \textit{before or in the year} of marriage
\end{enumerate}
\begin{itemize}
\item Around 1/4 marriages in the US today are kids-first
\item They have much higher divorce rates, especially among college graduates
\item Marginal in economic sense: could be sensitive to policies 
\end{itemize}
\end{frame}
...
\text{ \footnotesize (met and pregnant): \ } & + p^{\text{m}}_t \cdot p^{\text{p}}_t\cdot   \left[ m^p_t \cdot V^{f,ck}_{t+1}  + (1-m^p_t)\cdot  V^{f,\text{preg}}_{t+1}\right] \Big\} \Bigg\}, \\
\text{ \footnotesize (uncertain abortion access): } &  V^{f,\text{preg}}_{t+1} = p^{\text{access}} \cdot \max\{V^{f,sk}_{t+1},V^{f,s}_{t+1} - \phi_a\} \\
   & \hspace{1cm} + (1-p^{\text{access}})\cdot V^{f,sk}_{t+1},\\
